...
export class SeedService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    // Se resuelve al real UUID despues de crear los usuarios
    ...
    for (const seed of seedData) {
      ...
      this.logger.log(`  ${seed.username} creado`);
    }
    ...
    for (const apikey of seedApiKeys) {
      ...
      if (!realUserId) {
        this.logger.warn(`  No se encuentra user_id para customer_id: ${apikey.customer_id_olist}. Key expirada omitida.`);
        continue;
      }
      ...
      this.logger.log(`  Key expirada insertada para ${apikey.customer_id_olist}`);
    }
